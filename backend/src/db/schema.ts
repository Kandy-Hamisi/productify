...
// `one()` means one product can have only one related record, many() means multiple related records

export const productRelations = relations(products, ({ one, many }) => ({
  comments: many(comments),
  // `fields` = the foreign key column in THIS table (products.userId)
  // `references` = the foreign key column in the other table (users.id)
  user: one(users, { fields: [products.userId], references: [users.id] }),
}));

// Comments Relations: A comment belongs to one user and one product
// `one()` means one comment can have only one related record, many() means multiple related records

export const commentRelations = relations(comments, ({ one }) => ({
  // `comments.userId` = the foreign key, `users.id` = is the primary key
  user: one(users, { fields: [comments.userId], references: [users.id] }),
  // `comments.productId` = the foreign key, `products.id` = is the primary key
